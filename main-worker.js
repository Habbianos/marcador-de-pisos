self.onmessage = function(e) {
	...
	for (var i = 0; i < fonte.length; i++)
		for (var j = 0; j < fonte[i].length; j++)
			if (fonte[i][j].type == "◫" && (fonte[i][j].state == "0" || fonte[i][j].state == "2"))
				descobertosOrig.push({x: i, y: j});

	var menorQtd = descobertosOrig.length + 1;
	var cont = 1, completed = 0, total = fact(descobertosOrig.length);
	theShuffle: do {
		for (var i = 0; i < fonte.length; i++)
			for (var j = 0; j < fonte[i].length; j++)
				fonte[i][j].state = 0;
